  ...
  return (
    <div className="flex items-center flex-col w-full bg-[url(magicLogo.svg)]">
      ...
      <div className='w-full flex justify-center mb-4'>
        <p className='text-3xl font-Karantina text-orange-800'>Favorite Cards</p>
      </div>

      <div className="grid grid-cols-2 gap-3 w-5/6" >
        {selectedFavoriteCards.map((card) => (
          <div key={card.id} className="bg-no-repeat bg-cover font-Karantina rounded-2xl h-96" style={{ backgroundImage: `url(${card.image_uris?.art_crop})` }}>
            <div className='bg-orange-900/50 rounded-2xl w-full flex items-end mb-2 h-full'>
              <div className=' ml-3 mb-3'>
                <p className='text-orange-200 text-5xl -mb-2 font-Karantina'>{card.name}</p>
